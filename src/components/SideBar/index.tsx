...
export default function SideBar() {
  ...
  const askSubscription = (event: any, groupId: any) => {
    ...
    axios
      .post(`${apiUrl}/group_requests`, { targetGroup: groupId }, config)
      .then((res) => {
        setOpen(true);
      });
  };
      ...
      >
        {ownedGroups.includes(group["@id"]) && (
          <Tooltip title="Supprimer ce groupe">
            ...
            >
              <ListItemIcon>
                <DeleteIcon style={{ color: "tomato", cursor: "pointer" }} />
              </ListItemIcon>
            </ListItemButton>
          </Tooltip>
        )}
        {!subscribedGroups.includes(group["@id"]) && (
          <Tooltip title="Demander à rejoindre ce groupe ">
            <ListItemButton
              style={styleBtn}
              onClick={(e) => askSubscription(e, group["@id"])}
            >
              <ListItemIcon>
                <PersonAddAlt1Icon
        ...
        {subscribedGroups.includes(group["@id"]) && (
          <Tooltip title="Créer un thread">
            <ListItemButton>
              <ListItemIcon>
                <AddCircleIcon
                  onClick={() =>
                    navigate(`${group["@id"].replace("/api", "")}/threads`)
                  }
                  style={{ color: "#1876d1", cursor: "pointer" }}
                />
              </ListItemIcon>
      ...
      <List>
        {groups.map((group: Group, index) => (
          <ListItem key={index} disablePadding>
            <ListItemButton
              style={styleBtn}
              onClick={() => navigate(`${group["@id"].replace("/api", "")}`)}
            >
              <ListItemIcon>
                <WorkspacesIcon />
              </ListItemIcon>
  ...
  const list = (anchor: Anchor) => (
    ...
    >
      <List>
        ...
        <ListItem disablePadding>
          <ListItemButton
            sx={{ backgroundColor: "#1876d1", color: "white" }}
